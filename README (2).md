...
- **Modelli testati:** ResNet18 vs MLP Baseline.
...
- **Modelli testati:** ResNet18, MLP Baseline, MobileNetV2 e ViT_B_16.
- **Obiettivo:** Comprendere le difficoltà del modello sulle classi di transizione (es. la sottile differenza tra Mild e Very Mild).
...
3. **Data Augmentation Sensata:** Sono state applicate `Resize((224, 224))`, conversione in Tensore RGB e normalizzazione con media e deviazione standard di ImageNet. *Nota tecnica: Anche se le MRI sono in scala di grigi, la normalizzazione ImageNet sui 3 canali è un requisito matematico obbligatorio per non sfalsare l'attivazione dei pesi nel Transfer Learning. Nota clinica: Sono state volutamente escluse trasformazioni irrealistiche come il flip verticale (un cervello non si presenta mai sottosopra in una MRI).*
...
1. **MLP (Baseline):** Una rete fully-connected "pura". Prende in input i pixel grezzi dell'immagine flattati in un vettore 1D di 150.528 valori (224×224×3), distruggendo volutamente la struttura spaziale. È usata come termine di paragone per dimostrare matematicamente l'inadeguatezza dei layer lineari semplici sulle immagini rispetto alle convoluzioni.
...
- **Il caso ViT:** Pur essendo un modello State-of-the-Art, l'addestramento su hardware standard (GPU T4) si è rivelato computazionalmente insostenibile senza il totale congelamento dei layer. Costretto a un Early Stopping forzato, il modello ha restituito un F1-Score del **[INSERISCI VALORE]%**, inferiore a quello della ResNet18. Questo dimostra che i Transformer richiedono risorse e regimi di training (es. cluster multi-GPU) che non sempre giustificano il costo rispetto alle CNN classiche in task di queste dimensioni.
...
È doveroso segnalare una limitazione strutturale derivante dall'utilizzo dell'**Augmented Alzheimer MRI Dataset** di Kaggle. Poiché il dataset è stato pre-augmentato offline dall'autore originale, l'applicazione del `random_split` (seppur con seed fisso) introduce un inevitabile grado di **Data Leakage**: varianti augmentate dello stesso paziente potrebbero essere finite sia nel Train che nel Test set, gonfiando le metriche di accuratezza.

In un contesto clinico reale, lo split dei dati deve avvenire a livello di paziente (**Patient-Level Split**) prima di qualsiasi tecnica di augmentation. Ai fini di questo progetto accademico, le immagini sono state trattate come campioni indipendenti per focalizzarsi sull'architettura della pipeline MLOps.
